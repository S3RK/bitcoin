...
CWalletTx* CWallet::AddToWallet(CTransactionRef tx, const CWalletTx::Confirmation& confirm, const UpdateWalletTxFn& update_wtx, bool fFlushOnClose)
{
    ...
    WalletBatch batch(*database, false, fFlushOnClose);
...
void CWallet::MarkConflicted(const uint256& hashBlock, int conflicting_height, const uint256& hashTx)
{
    ...
    WalletBatch batch(*database, false, false);
...
DBErrors CWallet::LoadWallet(bool& fFirstRunRet)
{
    ...
    fFirstRunRet = false;
    DBErrors nLoadWalletRet = WalletBatch(*database, true).LoadWallet(this);
...
{
    AssertLockHeld(cs_wallet);
    DBErrors nZapSelectTxRet = WalletBatch(*database, true).ZapSelectTx(vHashIn, vHashOut);
